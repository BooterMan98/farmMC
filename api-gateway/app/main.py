...
def resolve_get_user(obj, resolve_info: GraphQLResolveInfo, id):
    response = requests.get(f"http://granja-service/users/{id}")
...
def resolve_list_users(obj, resolve_info: GraphQLResolveInfo):
    response = requests.get(f"http://granja-service/users/")
...
def resolve_list_plants(obj, resolve_info: GraphQLResolveInfo):
    response = requests.get(f"http://granja-service/plants")
...
def resolve_create_user(obj, resolve_info: GraphQLResolveInfo, userId):
    print(f"http://granja-service/users?id={userId}")
    return requests.post(f"http://granja-service/users?id={userId}").json()


@mutation.field("plant")
def resolve_plant(obj, resolve_info: GraphQLResolveInfo, userId, plantName, posX, posY):
    return requests.post(f"http://granja-service/plants?userId={userId}&plantName={plantName}&posX={posX}&posY={posY}").json()
...
def resolve_harvest(obj, resolve_info: GraphQLResolveInfo, construction_id, construction):
    ...
    return requests.post(f"http://granja-service/harvest/{construction_id}", json=payload).json()

@mutation.field("upgradeFarm")
def resolve_upgrade_farm(obj, resolve_info: GraphQLResolveInfo, userId):

    return requests.post(f"http://granja-service/upgradeFarm?userId={userId}").json()
